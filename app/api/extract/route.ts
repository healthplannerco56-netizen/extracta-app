...
// Per-user in-process rate limiter. Replace with Upstash/Redis for multi-instance.
...
export async function POST(req: NextRequest) {
  try {
   ...
   const { data: { user}, error } = await supabase.auth.getUser(token)
    ...
    if (typeof pdfText !== 'string' || pdfText.length > MAX_PDF_TEXT_CHARS) {
      return NextResponse.json(
        { error: `pdfText too large (max ${MAX_PDF_TEXT_CHARS} chars)` },
        { status: 413 }
      )
    }

    if (typeof fields !== 'string' || fields.length > MAX_FIELDS_LENGTH) {
      return NextResponse.json(
        { error: `fields too large (max ${MAX_FIELDS_LENGTH} chars)` },
        { status: 413 }
      )
    }
